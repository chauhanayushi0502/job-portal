...
function CompanyPanel() {
  ...
  // };
  const updateProfile = async () => {
    const phoneNumber = profileForm.phone ? profileForm.phone.trim() : "";
    const Location = profileForm.location ? profileForm.location.trim() : "";
    const Website = profileForm.website ? profileForm.website.trim() : "";
    ...
    if (!/^(?!\d+$)[a-zA-Z0-9\s,\.\-#/]{5,100}$/.test(Location)) {
      alert("Give valid Location.");
      setMessageType("error");
      return;
    }
    if(!Website){
    ...
    if(!/^(https?:\/\/)?([\da-z\.-]+)\.([a-z\.]{2,6})([\/\w \.-]*)*\/?$$/.test(Website)){
      alert("write correct link of our website");
    }
    try {
      setLoading(true);
      setMessage("");
  ...
  const addJob = async () => {
    ...
    if (!/^(?!\d+$)[a-zA-Z0-9\s,\.\-#/]{5,100}$/.test(jobForm.location.trim())) {
      alert("Please provide a valid location (cannot be only numbers).");
      return;
    }
    ...
    if(!/^(?!\d+$)[a-zA-Z0-9\s,\.\-#/]{2,50}$/.test(jobForm.category.trim())){
      alert("Please provide a valid category (cannot be only numbers).");
      return;
    }
    if (jobForm.salary.min && jobForm.salary.max && Number(jobForm.salary.min) > Number(jobForm.salary.max)) {
  ...
  const updateJob = async () => {
    ...
    try {
      ...
      const response = await fetch(
        getFetchUrl(`api/company/updatejob/${editingJob._id}`),
        {
          method: "PUT",
          headers: {
            token: token,
  ...
  const deleteJob = async (jobId) => {
    ...
    try {
      ...
      const response = await fetch(
        getFetchUrl(`api/company/deletejob/${jobId}`),
        {
          method: "DELETE",
          headers: {
            token: token,
